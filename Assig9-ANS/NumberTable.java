...
public class NumberTable implements UIButtonListener{
	...
	for (int row=0; row<this.table.length; row++){
	    double sum = 0;
	    for (int col=0; col<this.table[row].length; col++){
		sum += this.table[row][col];
	    }
	    UI.printf("Row %2d: %6.2f\n", row, sum);
	}
	// END OF YOUR CODE
    }
	...
	int cols = UI.askInt("how many cols: ");
	this.table = new double[rows][cols];
	for (int row=0; row<rows; row++){
	    for (int col=0; col<cols; col++){
		this.table[row][col] = UI.askDouble("("+row+","+col+"): ");
	    }
	}
	// END OF YOUR CODE
    }
	...
	try{
	    ...
	    for (int row=0; row<rows; row++){
		for (int col=0; col<cols; col++){
		    this.table[row][col] = sc.nextDouble();
		}
	    }
	    // END OF YOUR CODE
	    sc.close();
