...
public class Exercise implements UIButtonListener, UIMouseListener{
    ...
    */
	...
	try{
	    ...
	    // YOUR CODE HERE
	    while (sc.hasNext() && count<maxCount){
		tokens[count] = sc.next();
		count++;
	    }
	    sc.close();
    ...
    */
	...
	try{
	    ...
	    // YOUR CODE HERE
	    while (sc.hasNext() && count<maxCount){
		figs[count] = new CartoonFigure(sc.next(), sc.nextDouble(), sc.nextDouble());
		count++;
	    }
	    sc.close();
...
*/
	...
	try{
	    ...
	    sc.close();
	    double mid = nums[(count-1)/2];
	    for (int i=0; i<count; i++){
		if (nums[i]<mid)       { UI.setColor(Color.green); }
		else if (nums[i]==mid) { UI.setColor(Color.black); }
		else                   { UI.setColor(Color.red); }
		UI.fillRect(i*6, 0, 5, nums[i]);
	    }
	    // END OF YOUR CODE
	...
	for (int loop=0; loop<5; loop++){
	    ...
	    if (index>=0 && index < 5 && ducks[index] !=null){
		ducks[index].shoot();
		ducks[index] = null;
	    }
	}
